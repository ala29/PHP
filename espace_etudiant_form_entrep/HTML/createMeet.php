...
<section id="secondf">
		<h1>Remplir votre meet </h1>
					<!---image du connectee a mon comte--->
		<img class="inlinedisplay" id="imgcompte" src="../images/compte/5.JPG" alt="image compte"/>
					
		<form class="inlinedisplay" name="connectezVous" method="POST" action="createMeet.php" enctype="multipart/form-data">
			<legend> </legend>
					
			<div>
				<label for="id_meet">ID Meet <span id="etoile">*</span>:</label><br/>
				<input id="input" type="text" name="id_meet" id="id_meet"  autofocus required />
			</div>
					
			<div>
				<label for="titre_meet">titre_meet<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="titre_meet" id="titre_meet" required />
			</div>
            <div>
				<label for="description">description<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="description" id="description" required />
			</div>
            <div>
				<label for="date_meet">date_meet<span id="etoile">*</span>:</label></br>
				<input id="input" type="date" name="date_meet" id="date_meet" required />
			</div>
            <div>
				<label for="prix">prix<span id="etoile">*</span>:</label></br>
				<input id="input" type="number" name="prix" id="prix" required />
			</div>
            <div>
				<label for="nombre_participants">nombre participants<span id="etoile">*</span>:</label></br>
				<input id="input" type="number" name="nombre_participants" id="nombre_participants" required />
			</div>
            <div>
				<label for="ID_etudiant">ID_etudiant<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="ID_etudiant" id="ID_etudiant" value="<?= $_SESSION['id_etudformentrep'] ?>" readonly/>
			</div>
			<div>
				<label>choisir l'image de meet</label>
		...
		<?php
		require_once("createdMeet.php");?>
		
		<?php if($verificationM==2){?>
			<div class="msgall">
			<span class="msg m1">SUCCES! Meet crée</span></div>
		<?php }else	if($verificationM==1){?>
			<div class="msgall">
			<span class="msg">ECHEC! Meet existe!</span></div>	
		<?php }else if ($verificationM==0) {?>
			<div class="msgall">
			<span class="msg">veuillez remplir le formulaire!</span></div>
		<?php }else if ($verificationM==3){?>
				<div class="msgall">
				<span class="msg">etudiant formateur n'existe pas!</span></div>
			<?php }?>
	</section>
