...
<?php
		...
		$recherche = select('inscription',$_SESSION['id_etudformentrep'],'ID_etudiant');
		if($recherche==null){
			$ID_meet = $_REQUEST["id_meet"];
			$tab = array(
			"ID_meet" => $ID_meet,
			"ID_etudiant" => $_SESSION['id_etudformentrep'],
			);				
			insert('inscription',$tab);
      $pdo=connexionDB();
      try{
      $sql="UPDATE meet SET nombre_participants = nombre_participants+1 where ID_meet=".$ID_meet;
