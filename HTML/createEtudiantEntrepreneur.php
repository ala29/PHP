...
<body>
	...
	<section>
    <h1>Inscription Etudiant Entrepreneur</h1>
    ...
    <form class="inlinedisplay" name="connectezVous" method="POST" action="#">
        ...
        <div>
            <label for="ID_etudiant">ID étudiant <span id="etoile">*</span>:</label><br/>
            <input id="input" type="text" name="ID_etudiant" id="ID_etudiant" value="<?= $_SESSION['ID_etud']?> " autofocus readonly/>
        </div>
        <div>
            <label for="nom_projet">Nom projet <span id="etoile">*</span>:</label><br/>
            <input id="input" type="text" name="nom_projet" id="nom_projet" value="<?php if(!empty($_POST["nom_projet"])){echo $_POST["nom_projet"];} ?>" autofocus required />
        </div>
        <br/>
        <div>
            <input class="button button1" type="submit" name="connection" value="inscrit"/>
        </div>
        <?php
            require_once("createdEtudiantEntrepreneur.php");
            if($verificationEE==2){?>
                <div class="">
				<?php $_SESSION['id_etudentrep'] = $_SESSION['ID_etud']; ?>
                    <span class="">SUCCES! L'étudiant Entrepreneur est créé</span>
					<?php header("Location:../espace_etudiantentrepreneur/HTML/CreateProject.php");?>
				</div>
            <?php } else if($verificationEE==1){?>
                <div class="">
                    <span class="">ECHEC! L'étudiant Entrepreneur existe déjà!</span>
                </div>    
            ...
            <?php } else if($verificationEE==3){?>
                <div class="">
                    <span class="">ECHEC! L'étudiant n'existe pas</span>
                </div>
		<?php }?>
    </form>
