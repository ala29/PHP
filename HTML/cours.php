				...
				<div class="imagee"><img src="<?="../espace_etudiantformateur/HTML/".$meet->photo?>" alt="img_meet<?= $meet->ID_meet ?>" class="img_meet"></div>
					<div class="contentt">
						...
						<?php 
							  ...
							  $cinmet=$meet->ID_meet;
							  $Prix_Meet=$meet->prix;
							  $Etudiant_formateur= $rslt->prenom ." ". $rslt->nom;
							  $Date_Meet=$meet->date_meet;
							  
						?>
					<button class="buttonn buttonn1" type="submit" name="connection" ><a href="InscritMeet.php?id_meet=<?=$cinmet?>&id_etudiant=<?=$cinet?>&prix=<?=$Prix_Meet?>&etudf=<?=$Etudiant_formateur?>&datemeet=<?=$Date_Meet?>&meet=<?= $meet->titre_meet?>">Inscription</a></button>
