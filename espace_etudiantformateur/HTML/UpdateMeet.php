...
$rslt =select('meet',$_REQUEST["ID_meet"],'ID_meet');
?>

<section id="secondf">

		<h1>Modifier votre meet </h1>
					<!---image du connectee a mon comte--->
		<img class="inlinedisplay" id="imgcompte" src="../images/compte/5.JPG" alt="image compte"/>
		...
		<form class="inlinedisplay" name="connectezVous" method="POST" action="#">
			<legend> </legend>
			 ...
			 <div>
				...
				<input id="input" type="text" name="ID_meet" id="id_meet" value="<?=$_REQUEST["ID_meet"]?>" autofocus readonly />
			</div> 
					
			<div>
				<label for="titre_meet">titre_meet<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="titre_meet" id="titre_meet"  value="<?= $rslt[0]['titre_meet'] ?>" required />
			</div>

            <div>
				<label for="description">description<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="description" id="description"  value="<?= $rslt[0]['description'] ?>" required />
			</div>

            <div>
				<label for="date_meet">date_meet<span id="etoile">*</span>:</label></br>
				<input id="input" type="date" name="date_meet" id="date_meet" value="<?= $rslt[0]['date_meet'] ?>" required />
			</div>

            <div>
				<label for="prix">prix<span id="etoile">*</span>:</label></br>
				<input id="input" type="number" name="prix" id="prix" value="<?= $rslt[0]['prix'] ?>"required />
			</div>

            <div>
				<label for="nombre_participants">nombre participants<span id="etoile">*</span>:</label></br>
				<input id="input" type="number" name="nombre_participants" id="nombre_participants" value="<?= $rslt[0]['nombre_participants'] ?>" required />
			</div>
            
            <div>
				<label for="ID_etudiant">ID_etudiant<span id="etoile">*</span>:</label></br>
				<input id="input" type="text" name="ID_etudiant" id="ID_etudiant" value="<?= $rslt[0]['ID_etudiant'] ?>" readonly/>
			</div>
					<!---button--->
			<div>
				<input class="button button1" type="submit" name="connection" value="modifier"/>
			</div>
		</form>
	</section>

    <?php
    if(isset($_REQUEST['ID_meet']) && isset($_REQUEST['titre_meet']) && isset($_REQUEST['description'])&& isset($_REQUEST['date_meet'])&& isset($_REQUEST['prix'])&& isset($_REQUEST['nombre_participants'])&& isset($_REQUEST['ID_etudiant'])){
    require_once('requettes.php');
	$Updatedmeet = updateMeet('meet',$_REQUEST["ID_meet"]);
    ?>
    <?php
    if($Updatedmeet->rowCount()!=0){
    ?>
    <div class="msgall">
    <span class="msg m1">SUCCES! Meet Modifier </span></div>
