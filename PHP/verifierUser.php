...
$req_etudiant = $bdd->prepare('SELECT * FROM etudiant,etudiant_entrepreneur ee,etudiant_formateur ef WHERE email = :email AND mot_de_passe = :motdepasse AND etudiant.ID_etudiant=ee.ID_etudiant AND etudiant.ID_etudiant=ef.ID_etudiant');
$req_etudiant->execute([
    'email' => $emailx,
    'motdepasse' => $motdepassex
]);
$etudiant_form_entrep = $req_etudiant->fetch();
...
}elseif($etudiant_form_entrep){
    // interface pour l'étudiant formateur
    $_SESSION['id_etudformentrep']=$etudiant_form_entrep['ID_etudiant'];
    $_SESSION['prenom_etudformentrep']=$etudiant_form_entrep['prenom'];
    $_SESSION['nom_etudformentrep']=$etudiant_form_entrep['nom'];
    header("Location:../espace_etudiant_form_entrep/index.php");  
